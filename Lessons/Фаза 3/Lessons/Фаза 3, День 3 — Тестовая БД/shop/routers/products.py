...
@router.get("/", summary="Получит все товари")
def get_products():
...
def create_product(product: ProductSchemas):
    with Session(engine) as session:
...
def update_product(product_id: int, data: ProductSchemas):
    with Session(engine) as session:
        product = session.get(ProductModel, product_id)
        if not product:
            raise HTTPException(status_code=404, detail="Продукт не найден")
...
@router.delete("/{product_id}", summary="Удалит продукт")
