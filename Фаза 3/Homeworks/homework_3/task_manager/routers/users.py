...
router = APIRouter(prefix="/users", tags=["Ползователи"])
...
@router.get("/{user_id}", summary="Полуить пользовател по id!")
...
def create_user(user: UserSchemas):
    with Session(engine) as session:
        ...
        session.flush()
        session.commit()
        return {"msg": f"Пользовател {user.name} добавлено!", "id": new_user.id}

@router.put("/{user_id}", summary="Обновить пользователю!")
def update_user(user_id: int, data: UserSchemas):
    with Session(engine) as session:
        ...
        session.commit()
        return {"msg": f"Пользовател {user_id} обнавлён!"}
